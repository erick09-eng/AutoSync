...
class Sale(Base):
    ...
    document_type_id = Column(Integer,ForeignKey("document_types.document_type_id"),nullable=False)  # ForeignKey("document_types.document_type_id")
    serial_number = Column(String(100), nullable=False)
    customer_id = Column(Integer,ForeignKey("customers.customer_id"),nullable=False)       # ForeignKey("customers.customer_id")
    user_id = Column(Integer,ForeignKey("users.user_id"),nullable=False)           # ForeignKey("users.user_id")
    subtotal = Column(Float, nullable=False)
    tax_amount = Column(Float, nullable=False)
    discount_amount = Column(Float, default=0.0)
    total_amount = Column(Float, default=0.0)
    payment_method = Column(Integer,ForeignKey("payment_methods.payment_method_id"),nullable=False)
